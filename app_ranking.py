...
def create_ranking_poll(choices, worksheet, poll_name):
    ...
    for i in range(1, len(choices) + 1):
        ...
        rankings[i] = st.selectbox(f"{i}. Rang", available_choices, key=f"{poll_name}_{i}")
...
def main():
    ...
    polls = []
    predefined_worksheets = ["Umfrage 1", "Umfrage 2"]
    for i, q_a in enumerate(questions_and_answers):
        question = q_a[0]
        answers = q_a[1:]
        worksheet_name = predefined_worksheets[i] if i < len(predefined_worksheets) else f"Umfrage {i + 1}"
